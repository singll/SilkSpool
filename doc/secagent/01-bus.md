...
`data/bus.aliases.yaml`（bundle 模板版本受控）。**2026-09-19 起为空注册表**——全部 v4→v5 兼容别名已随「先迁调用方、再删别名」执行完毕：
...
3. 别名使用 → audit 记 `deprecated_use`（alias 字段标来源）；删除走废弃三段式。**当前无别名**（2026-09-19 全部移除）；
