...
**沙箱凭据隔离（2026-09-19 修复 M6）**：bwrap 不再整目录读写挂载 `$HOME`（原实现会暴露 `~/.ssh/id_ed25519`、`~/.config/fofa.conf` 及浏览器登录态）。现改为「HOME 内建 tmpfs + 只读投影工具链目录（`.config`/`.local`/`go`/`nuclei-templates`/`.cache`）+ 只读挂载 HOME 顶层常规配置文件」，凭据/私钥/浏览器 profile 不落盘可见；工具仍可读 `~/.config/<tool>/` 配置。
