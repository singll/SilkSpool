...
- **在办批次**：25 号方案 B1 大数据治理（42 号补丁）**代码完成、本地契约全绿，部署验收待执行**；B2 部分完成（索引/聚合缓存/批量投影；FTS 缓期），B3 升级时机待定——详见 [25 号方案 §4.5](25-dsh-0.1.7-upgrade-and-scale-2026-09-26.md)。
...
- **ledger 500 截断修复（指标口径恢复）**：`coverage_metrics`/`coverage_gaps`/`login_blindspot` 改全量分页遍历——资产 `asset_host_page`（20 万行）/ 端点 `endpoint_lite_page`（5 万行）/ findings `vuln_list`（2 万行）/ 根域 `asset_roots_agg` / 参数率 `endpoint_param_stats`（SQL 聚合），输出 `truncated` 标记；修复 bytedance crawl 分母 500/69605、param 率 24% vs 真实 35.7% 等失真。
...
- **验收**：本地契约全绿（bus54/asset32/endpoint33/vuln70/fact23/ledger31/task90/exec30/approval28/fgs21/scope18/report12/know78/eval30）；**部署验收待执行**。文档回填 [00-conventions](00-conventions.md)/[01-bus](01-bus.md)/[02-vuln](02-vuln.md)/[03-asset](03-asset.md)/[04-endpoint](04-endpoint.md)/[05-task §7.26](05-task.md)/[06-fact](06-fact.md)/[10-exec](10-exec.md)/[11-ledger](11-ledger.md)/[12-report](12-report.md)/[15-eval](15-eval.md)/[16-dashboard](16-dashboard.md)/[25 §4.5](25-dsh-0.1.7-upgrade-and-scale-2026-09-26.md)。
