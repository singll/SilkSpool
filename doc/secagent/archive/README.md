> **历史状态快照：2026-09-05 v4.7，2026-09-06 归档。**
> 当前入口：[文档导航](../README.md) · [v5 架构](../v5/README.md) · [实施进度](../v5/PROGRESS.md)。
> 历史升级文件于 2026-09-12 迁入 [upgrades](../upgrades/README.md)；本页只调整导航和链接，保留当时状态与实施结论。
> 下文“当前/待办/每次升版必做/仍有效”均是历史表述。具体接口、补丁和回滚方式以本次升级方案及最新检查为准。

---

# SilkSecAgent SRC 漏洞挖掘体系 · 持续推进文档

> 版本：v4.7 · 2026-09-05（由设计规范 v2.1 + 人工待办文档合并而来，为唯一持续推进入口；v4.7 = 外部仓库对标全量落地）
...
- **DSH 平台版本**：0.1.2-rc.1（2026-09-04 从 0.1.1-rc.2 升级，详见 [dsh-0.1.2-rc.1-upgrade-plan.md](../upgrades/dsh-0.1.2-rc.1-upgrade-plan.md) §十）；**0.1.3-alpha.1 不升**（SessionHandle 破坏性变更 + 官方性能回退，升级前置=两处 sessionPersistence 重写）；每次升版必做：重打 dsh-llm-deepseek reasoning_content patch + 确认 node_modules 属主；`dsh-version-watch.sh` 每日监控 dist-tags.latest
- 数据规模（2026-09-05 实测，v4.6.1 修复后）：assets **80,975**（未分级 1,785 待重跑 grade-assets）/ endpoints **106** / findings **65**（信号面 10 + 待验证候选 55）/ facts **1,143**（可见域；含 v4.6.1 迁入的 15 条 note: 工作记录）/ blackboard **46** 键（active **5 纯环境层**——快照键 active=0，v4.6.1 正则修复后 15 条 note: 键已全部迁 facts + archived 41）/ tasks **21** / approval_requests **7**（待审批 0）
...
- **外部仓库对标落地（v4.7，2026-09-05）**：对标 src-hunter-skill / Claude-Red / Claude-BugHunter 三仓库（方案见 `silksecagent-external-repos-benchmark-2026-09-05.md`）。① **验证纪律强化**——sec-verification 防误报纪律 8-16 条（OOB 回调才算数/时序采样 n≥10·2σ/高危双工具/三包对照/软404基线）+ never-submit 清单 + A→B 链信号表；② **严重度对抗性校准**——severity-rating 两段制（提交者自报不算数、复核者校准才算数，`sev: 自报→校准值` 轨迹）；③ **知识路由开局三步**——technique-index 头部路由（短表认现场→techniques/ 手法模块→VC 卡→kb_search），50 篇手法模块头部触发块密集化（触发信号+适用/不适用，avg 17.7 词）；④ **案例引用层**——rules/cases/ 22 篇全核实案例 + 补 path-traversal-lfi-test.md 漂移（technique-index 一直引用但文件缺失）；⑤ **守门三件套**——memcore R8 标识符泛化闸（scope.yml 域名/私网IP 命中即拒）+ R9 防膨胀（6000 字符）+ verifyExpRefs（source_ref 悬空→cooling）；⑥ **S5 写动词守卫**——scope-guard 第 5 步，passive 工具 argv 命中 33 写动词自动升 tool-intrusive 审批；⑦ **spawn_worker RoE 契约注入**——runWorker 共同咽喉注入授权边界锚块（双路径覆盖、幂等）；⑧ **阴性账本**——exp neg: 键 `neg:host|stack|class`（durable 90 天，永不静默缩减覆盖）；⑨ **覆盖度审计**——knowledge-coverage.py（36 攻面 × rules/vulncards 交叉表）+ 看板知识 tab 缺口卡；⑩ **评测框架**——eval-fp.js 假阳性消融（12 用例双条件跑增益）；⑪ **收割管道**——kb-harvest.py（feed/inbox/stdin → 分类 → 缺口比对 → 草稿骨架，绝不自动入库，配 knowledge-adopt 审批闭环）
...
- 设计要点：kind=scan-burst，payload 带 qps 目标值/时长窗口/资产范围；批准写 scope.yml defaults 的 burst_qps 字段（v4.6.1 起 run_cli 令牌桶直读 rate_limit_qps 且改值即时生效，最简落点=批准临时调高 defaults.rate_limit_qps、窗口过期回落），scheduler 在窗口内按新限速派发；窗口过期自动回落
...
够证即停（不拖数据/不横向）；证据脱敏；绝不用拖库凭据、绝不登真实用户账号；限速 50 QPS（✅ v4.6.1 已接执行点：active+ 工具 run_cli 启动走全局令牌桶，scope.yml 改值即时生效；批量临时升速仍走 T-16 scan-burst 审批）；intrusive 一律人工；蜜罐命中即退；规则变更即时生效。
...
| 日期 | 里程碑 | 记录 |
|---|---|---|
| 2026-09-05 | **v4.7 外部仓库对标全量落地（P0-P3）** | 对标 src-hunter-skill（信号路由表）/ Claude-Red（trigger-by-description）/ Claude-BugHunter（防误报纪律+对抗性校准+阴性账本+消融评测），方案文档 `silksecagent-external-repos-benchmark-2026-09-05.md`，P0-P3 十一项全落地。**P0 验证面**：sec-verification 防误报纪律 8-16 条（OOB 回调才算数/标记纪律/Body 差异/时序采样 n≥10·2σ/软404基线/高危双工具/三包对照）+ never-submit 清单 + A→B 链信号表（8 行）；severity-rating 对抗性校准两段制（提交者自报不算数复核者校准才算数、`sev: 自报→校准` 轨迹写 reason）；vuln-report-format 补「我未做的事」主动声明段 + 提交自检 6 项 + 平台差异（补天/CNVD 简化 + H1/Bugcrowd 英文结构）。**P1 知识面**：technique-index 开局路由三步（短表→手法模块→VC 卡→kb_search）+ 50 篇手法模块头部触发块密集化（avg 17.7 词/篇，纯插入零改动）；eval-fp.js + 12 用例假阳性消融评测（10 REJECT + 2 ACCEPT 双条件增益，报告落 data/eval/fp-report.json，LLM 走 Bellkeeper 网关 pool-secagent）；knowledge-coverage.py（36 攻面 TAXONOMY × rules/vulncards 交叉表，实测 36/36 有规则、15/36 有 VC 卡）+ 看板知识 tab 覆盖缺口卡（7 天缓存）。**P2 知识治理**：rules/cases/ 案例引用层 22 篇（全部来源 URL 经核实，目标泛化，USPS IDOR/Capital One SSRF/Samy XSS worm/PortSwigger 走输等；随 kbIndexCuratedRules() 自动进 FTS 零代码接入）+ 引用约束（引用案例必须 kb_search 命中实际条目）；memcore R8 标识符泛化闸（exp_cards 文本命中 scope.yml 域名/私网IP 拒绝，fail-open）+ R9 防膨胀（6000 字符）+ verifyExpRefs（source_ref 悬空→transition cooling）；experience.js 两处 validateWrite 调用捎带卡面文本激活 R8/R9；knowledge-adopt 审批 kind（validate 强制 draft 路径+source_url，onApprove 直写，enum 同步扩）；kb-harvest.py 收割管道 MVP（feed-url/inbox/stdin 三输入，25 类 TAXONOMY 分类，缺口比对，产出草稿骨架+候选清单，绝不自动入库）。**P3 纪律面**：S5 写动词守卫（scope-guard 第 5 步，33 动词表 + path 段级匹配，passive 工具命中写动词自动落 tool-intrusive 审批走白名单复用）；spawn_worker RoE 契约注入（ROE_ANCHOR/BLOCK 进 runWorker 共同咽喉，spawnWorker+scheduler 双路径覆盖，锚点幂等）；阴性账本（exp neg: 键规范 `neg:host=<host>|stack=<栈>|class=<手法族>`，durable 90 天，永不静默缩减覆盖 + review 复盘 neg 覆盖汇总）；persona v3→v4（vuln-hunt 路由三步+neg 查询、biz-logic 三包对照、review 校准+knowledge-adopt 提案）。**附带漂移修复**：path-traversal-lfi-test.md 补缺（technique-index 88-91 行一直引用但种子里不存在）。seed 计数 56→79 |
| 2026-09-05 | **v4.6.1 漂移审计发现的 6 项代码缺陷修复 + 部署链顺序缺陷** | ① **playbook 降级链路修复**：memcore recordSignal 此前不识别 pbOutcome 打点的 succeeded/ran 信号（「未知信号」被 try/catch 静默吞掉）且低成功率判定挂在已空的 playbooks 表上——现两信号被识别 + kind=playbook 卡降级判定（successRate 0.3/minRuns 5，沿用 playbooks 策略）；单测 13/13，线上 tool:dnsx（6 跑 0 成）保持 cooling；② **snapRe 正则修复**（alternation `note:` 后仍要求 `[:_]`，单冒号键永不匹配）→ 13:11 sweep 自动转写滞留的 15 条 note: 工作记录键入 facts（bb/ 前缀），黑板快照键 active=0、纯环境层达成；③ report_build 工具补 severity/source 参数（后端早已支持，agent 面传不进去）；④ **9 个管线脚本接线**（l2-collect/surface-consume/js-watch/ct-watch/pipeline-validate/coverage-report/verify-replay/dsh-version-watch/ct-watch-all）manifest templates + install_scripts 归位 + setup.sh §9.7 ct-watch.service 单元纳管（此前仅历史推送存活，全量重部署会丢）；⑤ AGENTS.md 受管区块生成文案主次颠倒修正（打法链沉淀主入口=pb_save，exp_store 无 kind 参数）；⑥ **QPS 限速接上执行点**：active+ 工具 run_cli 启动全局令牌桶（容量=defaults.rate_limit_qps，跨会话共享、改值即时生效）——此前纯声明值无代码读它；⑦ **setup.sh 重启顺序缺陷**（根因发现于本次部署实测 bbGuarded=0）：reconcile_service 此前排在主流程、§8 插件组装之前——服务带旧插件代码重启、新代码不生效，现移到 §10 收尾 |
| 2026-09-05 | **v4.6 知识体系按类型归一 + 看板知识全景** | 三合并一界面：① **合并①经验类归一**——playbooks 28 条迁入 exp_cards（kind=playbook 列 + runs/successes 列，pbOutcome 统计打点改写为卡分数反馈、pb_save/pb_rank 兼容期保留指回 exp_search，memcore 治理单套覆盖）；② **合并②事实类归一**——黑板快照键（recon:/alive:/scan:/review:/note: 共 41 条，当日实际迁移 26 条）迁入 facts（program_id=__legacy__ ephemeral 14d，幂等 meta 旗标）+ sweep 守卫防回潮（快照前缀新写入自动转 facts 并归档），黑板回归纯环境层（[env-issue]/[timeline]）——当日遗留 15 条 note: 键因 snapRe 正则缺陷未迁，**v4.6.1 当日修复**（见下）；③ **合并③文献类归一**——rules 56 篇建 curated 索引进 kb_docs（file 指向 rules/ 原路径不动、status=curated、不参与复验生命周期），kb_search 统一检索（curated 排序在前），techniques 从 grep 文件名收敛为 FTS 命中；④ 检索纪律三步顺序（fact_search→exp_search→kb_search）改写调度 prompt/AGENTS.md/sec-runtime-discipline rule 8；⑤ **看板知识 tab 重设计**：顶部六类型全景图（经验/事实/文献/规程/任务内/环境——每类一个位置一个工具）+ 新 KbSection 文献浏览（curated/external 筛选 + kbRead 正文 Modal）+ 经验卡表 kind 徽标 + RPC 新增 kbList/kbRead/factOverview；明确不合并（by design）：vulncards↔techniques、FGS↔facts、rules/src↔kb、CyberStrikeAI 库（T-17 维持） |
...
| 2026-08-28 | DSH 升级规划 + B0 执行 | [dsh-0.1.2-upgrade-arch-plan.md](../upgrades/dsh-0.1.2-upgrade-arch-plan.md) 产出；B0 排查全过（无 APIProxy 依赖）；**B1 暂缓：0.1.2-alpha.1 未发布 npm**（GitHub 仅源码），用户决策等 beta；已部署 dsh-version-watch 每日监控 npm |
...
- [dsh-upgrade-0.1.1-rc.2-report.md](../upgrades/dsh-upgrade-0.1.1-rc.2-report.md) — DSH 历史升级报告（2026-08-23；适用范围见升级目录）
