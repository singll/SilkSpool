...
> ⚠️ **文档状态（2026-08-28）**：本文是 **P11 时代的历史快照**（2026-08-22 校准）。§1.2 数据规模、§0 状态速览等均已过时（当前资产/任务/机制见 `README.md` 状态速览；系统完整解剖见 `silksecagent-system-complete.md`）。§三起的架构设计、踩坑记录（附录 A）与安全审计清单（附录 B）仍具长期参考价值，**运行时描述与本文冲突处一律以全景文档与运行代码为准**。
