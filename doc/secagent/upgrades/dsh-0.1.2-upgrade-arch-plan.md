> **历史升级规划：2026-08-28～09-01；平台目标已由后续 [0.1.2-rc.1 方案](dsh-0.1.2-rc.1-upgrade-plan.md) 替代。**
> 2026-09-12 从 archive/ 移入统一升级目录；保留原有架构增量和实施结论，不能据此认定部署过 alpha。
> 当前架构见 [v5](../v5/README.md)，历次升级见 [时间线](README.md)；正文仅调整导航和路径。

---

# DSH v0.1.1-rc.2 → v0.1.2-alpha.2 升级与架构优化规划

> 版本：v1.2 · 2026-09-01 ｜ 性质：**规划文档；B0/B3/B4/B5 已执行，B1 目标已改为 alpha.2**
> **升级状态（2026-09-01 更新）**：`0.1.2-alpha.1` 未发布 npm 二进制资产；`dsh-version-watch.sh` 于 2026-08-31 检测到 `0.1.2-alpha.2` 出现。B0 排查已完成（无 APIProxy 依赖）；B3 sec-pipeline 插件化、B4 sec-suite 拆分、B5 vault 同步均已于 2026-08-28 完成。**B1（DSH 本体升级）待用户决策是否升级到 alpha.2**，升级前需按 §一重新跑影响分析（alpha.2 release notes 可能新增变更）。
> 目标：① 升级 DSH 到 v0.1.2 且全部附加功能正常运行；② 借机对全部插件/脚本/提示词/流程做架构级优化，让每个部件处于最优位置；③ 知识产物（漏洞卡等）沉淀到 TrueNAS Obsidian vault。
> 关联：[历史持续推进文档](../archive/README.md)、[上次升级报告](dsh-upgrade-0.1.1-rc.2-report.md)、[领域语言](../../../bundles/dsh/CONTEXT.md)
...
6. [TrueNAS / Obsidian 沉淀方案](#六truenas--obsidian-沉淀方案)
