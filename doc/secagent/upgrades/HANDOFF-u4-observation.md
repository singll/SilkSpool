...
2. 别名闸口到期后，按宪法 §十五三段式删除 37 个别名；注意 [PROGRESS §5.2 侧记](../../v5/PROGRESS.md) 的契约用例 `freeform-status-update` 需同步改为直连语义动词，否则契约评测会报 `E_BUS_DOMAIN_UNKNOWN`。
3. 如需执行第 2 条推迟的 `rsync`/`bundle setup`，安排在维护窗口并记录重启基线。

> 自续规则：**不再适用。** 本链条（U4 观察期）已终止；后续工作按 [升级目录 README](README.md) 与 [v5 PROGRESS](../../v5/PROGRESS.md) 进行。
