...
>
> **v5 定位**：本文档是 **v3（原本，权威历史）＋ 安全审计（真相）** 的合流与收敛，v4（中间优化版本）已并入本 v5、不再单独保留——
> 1. [dsh-pi-secagent-plan.md](dsh-pi-secagent-plan.md)（v3）：P0–P5 落地历程与踩坑经验，是**权威历史记录**；
> 2. [dsh-secagent-security-audit-2026-08-21.md](dsh-secagent-security-audit-2026-08-21.md)：运行时 + 源码审计，判定**「文档声称的防护强度 > 代码实际强度」**。
>
> v5 做四件事：**① 合流成一份可执行主计划；② 明确两条能力主线（使用体验 / 发现漏洞的能力）；③ 确定「看板（Dashboard）」的设计（§六，本次 grill 定稿）；④ 给出分阶段优化计划与可落地执行步骤**。不推倒重来，全部增量、可回滚、数据无损。
> 执行完毕后，v3（历史）+ v5（计划）将合并为一份最终文档。

---

## 0. 一句话诊断

SilkSecAgent 已是一台**跑得通的引擎**：scope-guard fail-closed、token 压缩 1146x、692 资产 / 57 漏洞真实在库、靶场多引擎已跑通、美团/字节 SRC 实战各跑一轮。但它有三个短板：
...
3. **无看板**——看漏洞/资产/黑板只能靠会话里调工具 + markdown 报告，没有界面化入口。

**结论**：优化重心不是再造引擎，而是**给引擎装脊柱（Program→Task→Run）、装神经（事实图谱黑板）、装眼睛（看板 Dashboard）、补上漏装的护栏**——从「工具执行器」进化为「可管理、能自学、可交付的作战平台」。
...
| 声称 | 代码实况 | 处置 |
|---|---|---|
| manifest `parser`（jsonl/json/csv 结构化） | ❌ 只 regex 抽 URL/host（`ingestText`） | §九 步骤 1（P7）实现 parser 注册表 |
| **nuclei/afrog 结果入 findings 表** | ❌ 只有 xray-webhook / authz_diff 写 finding，**主扫描引擎结果不进漏洞库** | §九 步骤 1（P7）修复（最大缺口） |
| manifest `requires/produces` 能力凑链 | ❌ 字段写了没读 | §九 步骤 1（P7）实现 `plan_chain` |
| `summarize: head_llm` 分级摘要 | ❌ 永远 head-20 行 | §九 步骤 1（P7）+ 步骤 5（P9）自优化 |
| sandbox（bwrap/Landlock）隔离执行 | ❌ bwrap 装了没用（审计 S2，高） | §七 P10 接 dsh-sandbox |
| scope 解析后校验（DNS/重定向/回调） | ❌ 只有字面校验（审计 S1，高） | §七 P10 补后置校验 |
| egress-guard 出口白名单 | ❌ 未装（审计 S3，中） | §七 P10 接入 |
| taintguard 注入防护 | ❌ 未装（审计 S8，低） | §七 P10 接入 |
| 流量加密 + 保留期 + 脱敏 | ❌ 未落地（审计 S7，低） | §七 P10 接 output-retention |
| negative-ledger / chicheng-push | ❌ 未装 | §九 步骤 5（P9）内建 negative-ledger |
| pi-bridge 融合层（v3 §5.2） | ❌ 明确没做，走 DSH headless | §1.3 重定位（不再需要） |

### 1.3 DSH / pi / CyberStrikeAI 定位结论（核实到源码级）

- **DSH**（`deepseek-ai/deepseek-harness`，50 包，Cordis「一切皆插件」）：**主框架**。`goal/plan/todo` 是**会话内自编排构件**（易失），**不是**带独立生命周期的领域实体 → engagement/task/vuln **必须自建持久层**（挂 `ctx.<key>` Cordis service + 我们的 SQLite）。可直接复用：`ctx.jobs` / `ctx.workflowEngine` / `ctx.subagents` / `ctx.sessionQuery` / `ctx.approval` / `ctx.storage` / `ctx.credentials` / `ctx.spillStore` / `ctx.compaction`。**Web UI 是 React 前端 + slot 扩展机制（`dsh-client-ui-slots`）+ Host↔Client 数据通道（`dsh-api-remotes`/`dsh-api-gateway`）——看板据此实现（§六）。**
...
- **CyberStrikeAI**（`Ed1s0nZ/CyberStrikeAI`，已退役，数据保留在 `/opt/silkspool/dsh/data/import-staging/cyberstrikeai/`）：**只借鉴设计，不照搬实现**。四点直接借用：① 一切以 **Project 为作用域**（`project_id` 外键贯穿）；② **事实图谱**（category/body/confidence + 关系边）替代扁平 key:value；③ **orchestrator 三模式派单 + 交接包纪律**（子代理禁再委派）；④ **vulnerabilities 表即报告模板** + 组件触发搜洞触发器。**其"前端看板"的体验由我们自建 DSH 看板复刻，但不照搬其 Go 单体。**
...
| **进度可见** | 无看板，会话里翻 | 看板三视图（漏洞 / 资产 / 黑板，任务视图后置），进度/风险/盲区一眼可见 |
| **盲区可见** | 无 | 覆盖盲区：从未扫描 / >30 天未扫描按 `last_seen` 顶到最前 |
| **报告** | `report_build(host_like)` 只能按主机过滤 | `report_build(program)` 项目级 markdown，提交前强制人工审 |
| **判定打标** | 手工改状态 | 看板每个 finding 旁「确认/误报/风险接受」一键 = 一条评测用例 + 一条负样本（零摩擦） |
| **纠错** | 无界面，靠会话 | 事实纠正双通道：看板按钮（主）+ 对话自然语言（辅，结构化工具落地） |
...
| 象限 | 现状 | 目标 | 关键差距 |
|---|---|---|---|
| **A 技术栈漏洞**（模板/POC 扫描） | nuclei/afrog/xray 三引擎已跑，但**命中不进 findings 表** | parser→findings + 三级漏斗，命中结构化入库存证，规则层挡 60%+ 误报 | 最大缺口（§九 P7） |
| **B 越权/业务逻辑** | `authz_diff` 单发可用，但**接口图谱无鉴权建模**，无法批量 | 接口 `params/auth_required/roles_seen` 建模 → 按角色矩阵批量跑 authz_diff | 缺接口鉴权建模 + 批量流水线（§九 P8） |
| **C 代码审计/供应链** | semgrep/codeql/gitleaks/osv 已装，但与本机目标耦合、scope 无 target_param | 代码审计走独立 Preset + 结果入 finding（vuln_type/endpoint_ref 报告字段） | 中等（§九 P7-P8） |
...
**新增的眼睛**（§六）：看板（Dashboard）——DSH Web UI 内的 slot 插件，经原生 Remote 读库 + 受控写（打标/事实纠正），解决「看数据靠会话翻」的痛点。
...
1. **【最高优先】安全加固**（审计 9 项发现，§七）——先堵「文档强于代码」的误判防护面；
2. **【P7 补血】** parser 注册表 + nuclei→findings + plan_chain + 分级摘要——把「引擎结果进不了库」堵上，最大杠杆，也是「看漏洞」看板的前置；
3. **【看板 MVP】**（§六）——资产/黑板视图（随 P7）+ 漏洞视图/打标（P7 后），命中「使用体验」主线；
4. **【P6 脊柱】** programs/tasks 表 + task_* 工具 + program_id 回填 + orchestrator——「看任务」的前置；
5. **【P8 管理增强】** 事实图谱 + 指纹/凭据/接口鉴权建模 + finding 报告模板 + 盲区仪表盘 + 越权矩阵 + 任务视图——越权系统化 + 项目级报告；
6. **【P9 自学习】** negative-ledger 内建 + 经验卡语义去重 + 活评测集 + RAG rerank + component-vuln-intel——闭环自动生长。
...
> **本次 grill 定稿的 5 个决定**：① 融入 DSH Web UI（slot 插件）；② 数据走原生 Remote；③ 视图顺序 漏洞+资产→黑板→任务；④ 写操作=打标+事实纠正双通道；⑤ 增量交付、P7 提前于 P6。

### 6.1 承载方式：融入 DSH Web UI（客户端 slot 插件，不自建）

DSH Web UI 本身是 React 应用，`dsh-client-ui-slots` 提供 **slot 注册表**，插件一次 `register({ name, ... }, Component)` 即可往侧栏/工作区挂自定义面板；生态里的 dsh-bill / dsh-auth-gate / dsh-knowledge-base 均如此挂 UI。收益：**复用 auth-gate 鉴权 + 单一入口**（用户本来就在 DSH 里开会话），零额外端口/部署面——这正是「简洁、实用」的落点。详见 ADR `docs/adr/0002-secagent-dashboard-in-dsh-webui.md`。

### 6.2 数据通道：原生 Remote（api-remotes，只读 + 受控写）

浏览器不能直读服务端 SQLite。数据走 DSH 原生 Host↔Client 通道：**服务端插件注册 Remote 方法**（`dsh-api-remotes`），**客户端经 `ctx.remote` 调用**，传输走 `dsh-api-gateway`（同源 + 网关校验 + Agent/Session 身份策略）。只暴露**只读查询 + 两个受控写**，绝不暴露任意写。
...
| 视图 | 数据源 | 交付时机 |
|---|---|---|
| **漏洞** | `findings`（含 severity/status/vuln_type） | **P7 后**（findings 有活水） |
| **资产** | `assets`/`endpoints` | **P7 期间**（692 资产真实在库） |
| **黑板** | `blackboard`（简版）→ `facts`（P8） | **P7 期间简版** |
| **任务** | `tasks` | **P6 后** |
...
| 写操作 | 通道 | 理由 |
|---|---|---|
| **打标**（`finding_update`: confirmed/false_positive/ignored） | 看板按钮 | 封闭枚举、幂等、已有校验；= 活评测集反馈 |
| **事实纠正** | **双通道**：黑板视图「纠正/废弃」按钮（主）+ 对话自然语言 → agent 结构化 `fact_correct`/`fact_deprecate`（辅） | fact 错误会注入未来所有 prompt，必须有人工纠错口；两通道共用同一底层函数 + audit.jsonl |
| 资产/端点/凭据 | **不开写** | 由工具输出派生，错了就重跑工具；硬写制造与工具输出的双轨漂移 |

### 6.4 技术落点（host / client 两面）

DSH 的 Host（服务端）插件与 Client（浏览器）插件是**两套构建面**（`tsconfig.host.json` / `tsconfig.client.json`），现有插件全是 host 面。落点：

- **Host 面（复用现有 sec-suite）**：新增查询 Remote（`assetGraph.*`/`findingQuery`/`factList`）+ 写 Remote（`findingUpdate`、`factCorrect`/`factDeprecate`）——底层就是 `assetDb` 函数，sec-suite 已 import，**零新增依赖**；
- **Client 面（新增 `@silksec/sec-dashboard`）**：React slot 组件，经 `ctx.remote` 调用上述方法，渲染四视图 + 打标按钮 + 事实纠正按钮；
- **纪律**：pin 版本 + shim 收敛 + 先扫后装（沿用现有插件治理），client 插件走 client 构建面。

**关键纪律**：写路径**直接复用 agent 现有那套代码**（`assetDb.updateFinding` 等），不另写一份——保证「一份校验、一条 audit.jsonl、一个真相源」，杜绝双轨漂移。

### 6.5 增量交付顺序

1. **资产视图 + 黑板简版 + 事实纠正**（P7 期间，数据真实，独立于脊柱）；
2. **漏洞视图 + 打标按钮**（P7 后，findings 有活水了才有意义）；
3. **任务视图 + 起任务写操作**（P6 后）；
4. **黑板视图升级 facts 图谱 + 盲区仪表盘**（P8）。

### 6.6 本次落地（全局 / 会话分离，2026-08-21 定稿）

> 原 §6.1–6.4 的「看板挂 `conversation.view`（会话标签页）」方案已修正：全局数据不该挂在每个对话下。最终落地为**全局入口**（详见 ADR-0003）：
> - **入口** `sidebar.footer.action`（`scope:root`，侧边栏底部，任何会话/无会话时都可见）→ primitives `Modal`（headless，~1120px），四视图 tab：**漏洞 / 资产 / 事实 / 任务**。
> - **会话页**只保留会话级内容（消息 / Trajectory / Run）；Run 不进全局面。
> - 数据通道 `/silksec-dashboard` RPC 不变；写操作（打标 / 事实纠正）随看板进全局面，复用 `assetDb` 函数 + `audit.jsonl`。

**挂起清单（后续一起做，勿丢）**：

| # | 挂起项 | 依赖 / 归属 |
|---|---|---|
| 1 | **会话范围指示**（会话页显示「本会话工作范围 / 风险上限」） | 需先补 run→session 映射 |
| 2 | **跨面跳转**（全局 finding → 反查并跳回「哪条会话哪个 run 打出来的」） | 需先补 run→session 映射 |
| 3 | **盲区仪表盘**（从未扫描 / >30 天未扫描按 `last_seen` 顶到最前） | P8 |
| 4 | **指纹 / 凭据 tab**（`fp_add` 有活水后加第五个 tab） | P9 |
| 5 | **打标 → eval-cases 自动成评测用例**（`finding_update` 时追加 `eval-cases.list`） | P9 |

---

## 七、安全加固（整合审计 §2/§3，最高优先）
...
| + | 看板写操作鉴权 | ✅ 复用 auth-gate + Remote 身份策略 | 单 admin 暂不细分角色；起任务写操作（P6 后）仍过 scope-guard |
...
| **看板 MVP** | sec-suite 查询/打标/纠正 Remote + `@silksec/sec-dashboard` 客户端插件（资产+黑板简版视图，P7 后补漏洞视图+打标） | 资产/黑板一眼可见；漏洞命中 P7 后进看板；打标按钮=活评测用例 |
| **P6 脊柱** | programs/tasks 表 + task_* 工具 + program_id 回填 + orchestrator Preset | 「对 X 项目起 recon 任务」跑通；存量 692 资产回填项目归属 |
| **P8 管理增强** | 事实图谱 + 指纹/凭据/接口鉴权建模 + finding 报告模板 + 盲区仪表盘 + 越权矩阵 + 任务视图/起任务 | 越权按角色矩阵批量；项目级报告；资产风险自动算 |
| **P9 自学习强化** | negative-ledger 内建 + 经验卡语义去重 + 活评测集 + RAG rerank + component-vuln-intel | 证伪路径拦重复；实战打标自动成评测；指纹命中自动生成验证任务 |
| **P10 合规补齐** | sandbox / egress-guard / taintguard / output-retention / 解析后校验 | §七对照表全绿；审计 S1/S2/S3/S7/S8/S9 关闭 |

> **排序要点**：P7 提前于 P6——「看漏洞」依赖 P7、「看任务」依赖 P6，而核心诉求是先看漏洞；看板 MVP 横跨 P7→P6→P8 增量生长，不是单独一个大阶段。

---

## 九、执行步骤（可操作清单）

> 每步给出「做什么 / 改哪个文件 / 怎么验收」。顺序执行，每阶段结束可独立验证、可回滚。
...
3. 本文档 §七对照表即「文档标真实状态」的交付物（已完成）。

### 步骤 1（P7 补血，最高 ROI，看板「看漏洞」的前置）
...
1. **Host 面**：sec-suite 新增查询 Remote（`assetGraph.*`/`findingQuery`/`factList`）+ 写 Remote（`findingUpdate`、`factCorrect`/`factDeprecate`），底层直接调 `assetDb` 现有函数。
2. **Client 面**：新增 `@silksec/sec-dashboard` 客户端插件（slot 组件），`ctx.remote` 调用；先做**资产视图 + 黑板简版 + 事实纠正按钮**。
3. **P7 完成后**：补**漏洞视图 + 打标按钮**（findings 有活水）。
4. 打包：`templates/` 新增 client 插件产物 + setup 脚本装进 client 插件清单，走 pin + shim + 先扫后装。
- **验收**：看板内资产/黑板一眼可见；P7 后漏洞命中进看板；打标按钮触发 `finding_update` 且入 audit + eval-cases。
...
5. 看板：**任务视图 + 起任务写操作**；黑板视图升级为 facts 图谱；覆盖盲区仪表盘。
- **验收**：一个 biz-logic 任务覆盖全部鉴权接口；`report_build(program)` 出项目级报告；资产风险 `risk_score` 自动算；看板四视图齐。
...
- **验收**：§七对照表全绿；审计 S1/S2/S3/S4/S7/S8/S9 关闭。
...
| 能力 | 验收指标 |
|---|---|
| 使用体验 | 新项目 → 首份可审报告 ≤ 3 次点击/一条命令；看板四视图可看；盲区一眼可见；打标/纠错零摩擦；HITL 推手机 |
...
- **看板依赖 rc.7 内部 API**：slot / api-remotes 是预览版内部面，会随版本漂移——由 pin + shim 纪律兜底（客户端插件同样 pin），最坏重写 client 薄层，数据层（assetDb）不动；
- **自建 vs 复用边界清晰**：领域实体（program/task/finding/fact）自建持久层，编排/审批/检索/LLM/UI 复用 DSH+pi 底座，DSH 大版本只冲击 shim（最坏 1–2 天重构，数据无损）；
...
*本文档为 v3 落地历史 + 安全审计真相 + v4 优化设计（已并入）的合流主计划。v3 的 P0–P5 历程与踩坑仍是权威历史；审计报告仍是合规修复的核对清单；看板设计（§六）为本次定稿。执行完毕后，v3（历史）+ v5（计划）合并为一份最终文档。全部增量、可回滚、数据无损。*
