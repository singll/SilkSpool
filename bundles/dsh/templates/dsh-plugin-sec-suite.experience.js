...
export function apply(ctx) {
  ...
  // v5：exp_*/kb_*/pb_* 工具由 know 域接管（子仓前缀豁免，同名直通，ToolProjector 投影）。
  // 此处 v4 工具注册移除避免同名冲突；exp_validate 折叠进 exp_feedback(verdict=validated)（bus 别名）。
  // 底层函数（expStore/expSearch/kbImport/pbSave/pbOutcome/pbRank 等）保留——kbVaultSync 等内部调用方
  // 观察期内仍走 v4 函数路径。
}
