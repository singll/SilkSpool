...
# 逻辑漏洞测试手册
> **触发信号**: 逻辑漏洞, 价格篡改, 数量篡改, 负数, 整数溢出, 优惠券, 积分, 验证码, 万能验证码, 短信轰炸, 账号枚举, 密码找回, 接管, 状态机绕过, debug, is_admin, role=admin, orderId, amount, 重复提交
> **适用**: 支付/优惠券/验证码/找回/状态机这类业务规则绕过与账号安全逻辑 · **不适用**: 并发时序导致的问题（单发不成立时走 race-condition-test.md） · 索引: rules/src/technique-index.md
