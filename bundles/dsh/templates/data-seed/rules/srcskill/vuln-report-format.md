...
| 我未做的事 | 报告末尾（修复建议之后）加一段「我未做的事」：主动声明没打过的边界（如"未实际退款/未删除任何数据/未进任何账号，仅证明可达成"）。审核员 30 秒判风险用，也是自我保护——做了没写的危险动作比写了更糟。 |
| 提交自检 | 落盘后提交前过一遍：标题含站点+做成了啥且无「未登录」误用；危害每条有实值例子；每步恰好一个整包、能直接粘进 shell 重放；Cookie 与挖的时候一致（没偷偷摘掉装匿名）；密钥过了认钥闸；不提交清单（sec-verification 第 15 条）一条没踩。任一条不过 → 不交。 |
| 平台差异 | 本版式按国内 SRC（美团/字节口径）校准。交补天/CNVD：同内容去掉排版硬约束（全角＃分节等），标题+URL+等级+描述+复现+危害六段即可。交国际平台（H1/Bugcrowd）：整包转英文，结构改 Summary（含 CVSS 向量）/Steps to Reproduce（每步一个完整 HTTP 请求）/Impact（第一句就是影响，禁 "could potentially"）/Supporting Material，正文的「我」「大白话」腔改为客观陈述但 impact 仍举实值。 |
