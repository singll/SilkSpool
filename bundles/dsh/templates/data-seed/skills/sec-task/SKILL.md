...
5. **改调度用 task_schedule，补跑用 task_run_now**，取消用 task_cancel。
6. **定时任务里 exec_spawn_worker 派单纪律**：父 worker 的执行预算是硬上限（默认 3600s 到点 SIGKILL），
...
7. 定时任务的执行由 exec_spawn_worker 完成，执行会话自动归入对应工作区，结果写在任务 result + 执行历史里（看板可跳链查看）。
